...
// Server ishga tushganda avtomatik migrate
  ...
  try {
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS users (
        ...
        password VARCHAR(255), google_id VARCHAR(255),
        avatar_url TEXT, bio TEXT, level VARCHAR(20) DEFAULT 'beginner',
        xp INTEGER DEFAULT 0, push_token TEXT,
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS photos (
        ...
        appwrite_file_id VARCHAR(255) NOT NULL, image_url TEXT NOT NULL,
        caption TEXT, tags TEXT[], likes_count INTEGER DEFAULT 0,
        comments_count INTEGER DEFAULT 0, ai_analyzed BOOLEAN DEFAULT FALSE,
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS likes (
        ...
        created_at TIMESTAMP DEFAULT NOW(), UNIQUE(user_id, photo_id)
      )`);
    await client.query(`
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS ai_analyses (
        ...
        photo_id UUID REFERENCES photos(id) ON DELETE CASCADE,
        user_id UUID REFERENCES users(id) ON DELETE CASCADE,
        composition_score INTEGER, lighting_score INTEGER, overall_score INTEGER,
        composition_feedback TEXT, lighting_feedback TEXT, color_feedback TEXT,
        suggestions TEXT[], created_at TIMESTAMP DEFAULT NOW()
      )`);
    await client.query(`
      CREATE TABLE IF NOT EXISTS user_challenges (
        id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
        user_id UUID REFERENCES users(id) ON DELETE CASCADE,
        challenge_id VARCHAR(50) NOT NULL, completed_at TIMESTAMP DEFAULT NOW(),
        UNIQUE(user_id, challenge_id)
      )`);
    await client.query(`
      CREATE TABLE IF NOT EXISTS user_settings (
        user_id UUID PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
        notifications BOOLEAN DEFAULT true, ai_tips BOOLEAN DEFAULT true,
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS notifications (
        ...
        type VARCHAR(20) NOT NULL, text TEXT NOT NULL,
        photo_id UUID REFERENCES photos(id) ON DELETE CASCADE,
...
// Daily challenges jadvallar
const runDailyMigrations = async () => {
  const client = await pool.connect();
  try {
    await client.query(`
    ...
    `);
    await client.query(
      `CREATE INDEX IF NOT EXISTS idx_daily_ch_level_date ON daily_challenges(user_level, challenge_date)`,
    );
    await client.query(
      `CREATE INDEX IF NOT EXISTS idx_user_daily_ch ON user_daily_challenges(user_id, challenge_date)`,
    );
    console.log("✅ Daily challenges jadvallar tayyor");
  } catch (err) {
    console.error("Daily migration xato:", err.message);
  } finally {
    client.release();
  }
};
